...
class ItemBasedCF:
    ...
    #给用户user推荐前N个最感兴趣的物品
    def Recommend(self,user,K=3,N=3):
        rank = dict() #记录user的推荐物品（没有历史行为的物品）和兴趣程度
        action_item = self.train[user]     #用户user购买的物品和兴趣评分
...
def ItemCF(data,who):
    ...
    Item.ItemSimilarity()
    recommedDic = Item.Recommend(who)
    #计算给用户的推荐列表
    return recommedDic
# who = input()
# recommedDic = ItemCF(a,str(who))
# for k,v in recommedDic.items():
#     print (k,v)
# print(recommedDic.keys())